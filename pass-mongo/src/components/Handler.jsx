...
//         setPasswordArray([...passwordArray, { ...form, id: uuidv4() }]); // Use callback form of setState
...
//         if (c) {
//             setPasswordArray(passwordArray.filter(item => item.id !== id)); // Update state

//             // Send request to server to delete password
//             await fetch("http://localhost:3000/", { method: "DELETE",headers: 
//             { "Content-Type": "application/json" }, body: JSON.stringify({ id: form.id}) });
//             console.log(`password id ${id} deleted`)
...
const Handler = () => {
    ...
    const handleSearch = () => {
        // Filter passwordArray based on searchTerm
        const filteredPasswords = passwordArray.filter(password => password.URL.includes(searchTerm));
            ...
            <div className=" p-2 md-p-0 md:mycontainer">
                ...
                <div className="password">
                    ...
                    <div className="flex gap-2 items-center">
                        <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder='Search by URL' type="text" className='rounded-full border border-violet-500 w-full p-3 py-1' />
                        <button onClick={handleSearch} className='flex justify-center items-center bg-violet-300 rounded-full px-4 py-1 m-auto border-3 hover:text-white gap-2 font-bold hover:bg-violet-500 hover:border-violet-950'>
                            <lord-icon
